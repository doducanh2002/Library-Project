...
class DocumentServiceTest {
    ...
    void setUp() {
        ...
        testDocument = Document.builder()
                ...
                .bucketName("library-documents")
                .objectKey("documents/123456_test.pdf")
    ...
    void testUploadDocument_Success() {
        ...
        when(minioService.uploadFile(any(MultipartFile.class), anyString())).thenReturn("documents/123456_test.pdf");
        ...
        verify(minioService).uploadFile(any(MultipartFile.class), eq("documents"));
    }
    ...
    void testSearchDocuments() {
        ...
        when(documentRepository.findAll(any(), eq(pageable))).thenReturn(documentPage);
        when(documentMapper.toDTO(testDocument)).thenReturn(testDocumentDTO);
        when(accessControlService.canAccessDocument(testDocument)).thenReturn(true);
    ...
    void testGetDownloadUrl_Success() {
        ...
        when(documentRepository.findById(1L)).thenReturn(Optional.of(testDocument));
        when(accessControlService.canAccessDocument(testDocument)).thenReturn(true);
        when(minioService.generateDownloadUrl(anyString(), anyInt())).thenReturn("https://minio.example.com/download-url");
        ...
        // Assert
        assertNotNull(url);
        assertEquals("https://minio.example.com/download-url", url);
    ...
    void testGetMostDownloadedDocuments() {
        ...
        // Act
        var result = documentService.getMostDownloadedDocuments(5);
